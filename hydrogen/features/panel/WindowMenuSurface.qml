...
PanelWindow {
    ...
    readonly property var allGroups: windowStore.groups(modelData.name, workspaceName)
    readonly property var entries: overlayStore.panel === "window_group" ? windowStore.windowsForGroup(modelData.name, workspaceName, String(overlayStore.context.groupKey || "")) : Navigation.splitOverflow(allGroups, Math.max(1, Math.min(10, Math.floor((modelData.width - 220) / 42)))).overflow
        ...
        ListView {
            ...
            delegate: Rectangle {
                ...
                Image {
                    ...
                    width: 24
                    height: 24
                    source: Quickshell.iconPath(root.overlayStore.panel === "window_group" ? entryButton.modelData.identity.icon : entryButton.modelData.icon, "application-x-executable")
                }
                Text {
                    anchors.left: parent.left
                    ...
                    text: root.overlayStore.panel === "window_group" ? entryButton.modelData.title : entryButton.modelData.name
                    color: "#f4f7fb"
                    font.pixelSize: 13
                }
                Rectangle {
    ...
    function activateEntry(entry) {
        if (root.overlayStore.panel === "window_group") {
            ...
            root.overlayCoordinator.showWindowGroup(root.modelData.name, root.workspaceName, entry.key);
        }
    }
}
